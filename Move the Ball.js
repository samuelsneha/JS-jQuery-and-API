...
function setValue(value) {
    return value + "px";
    //that one dimension ( x = top OR  y = left ) which was changed, that value is updated and returned
}
...
function keyCode(keyPress) {
    ...
    // W is pressed - to move to up
    if (keyPress === 119 || keyPress === 87) {
        if (top > 5) {// this 5 is for buffer
            ball.style.top = setValue(top - 5);
        }
    }

    // A is pressed - to move to left
    else if (keyPress === 97 || keyPress === 65) {
        if (left > 5) { // this 5 is for buffer
    ...
    // this condition is basically the top from the previous iteration, should be less or equal to total screen height + ball height which is alsways fixed( so that it can occupy the entire height) + buffer (5 over here)
            ball.style.top = setValue(top + 5);
        }
    }
